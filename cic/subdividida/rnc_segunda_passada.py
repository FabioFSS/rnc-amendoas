...
if not os.path.isdir('net_results/second-pass/results/'):
    os.makedirs('net_results/second-pass/results/')
...
result_dir = f'net_results/second-pass/results/{str(str_time)}/'
os.mkdir(result_dir)


# Definindo hiperparametros

image_size = (120, 230) # Resolução da imagem
...
n_classes = 6 # Número de classes
class_mode = 'categorical'
train_dir = 'multiple_outputs_datasets/dataset2/train/' # Pasta do dataset de treino
test_dir = 'multiple_outputs_datasets/dataset2/test/' # Pasta do dataset de teste
...
kernel_regularizer=keras.regularizers.l2(0.01), kernel_initializer=kernel_init,
...
rede.add(Dense(units=32, activation=activ_func)) # Adiciona uma camada de classificação
rede.add(Dropout(0.3)) # Adiciona uma camada de dropout
rede.add(Dense(units=16, activation=activ_func)) # Adiciona uma camada de classificação
rede.add(Dropout(0.3)) # Adiciona uma camada de dropout
rede.add(Dense(units=8, activation=activ_func)) # Adiciona uma camada de classificação
rede.add(Dropout(0.3)) # Adiciona uma camada de dropout
...
rotation_range=0.2, validation_split=0.2)
...
# Fine tuning
best_model.compile(   optimizer=keras.optimizers.Adam(1e-5),  # Low learning rate
    loss=keras.losses.CategoricalCrossentropy(from_logits=True),
    metrics=[keras.metrics.CategoricalAccuracy()]
    )

epochs = 50
best_model.fit(training_set, epochs=epochs, validation_data=validation_set)
best_model.save(result_dir+'fine_tuned_model.h5')